...
def luminosity_distance(z, hubble_fn):
    ...
    integrand = c / hubble_fn(z_vals)
    D_C = np.trapz(integrand, z_vals)
    return D_C * (1 + z)
...
def main():
    ...
    plt.errorbar(data['z'], data['mu'], yerr=data['dmu'], fmt='o', label="Observed Data", capsize=3)
    plt.plot(data['z'], data['mu_empty'], label="Empty Universe")
    plt.plot(data['z'], data['mu_matter_only'], label="Matter-only Universe")
    plt.plot(data['z'], data['mu_lambda_only'], label="Lambda-only Universe")
    plt.plot(data['z'], data['mu_mixed'], label="Mixed Universe (Ω_m=0.3, Ω_Λ=0.7)")
